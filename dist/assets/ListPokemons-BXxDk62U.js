import{e as re,f as w,B as _,s as U,c as v,o as i,a as r,m,g as ae,R as ie,h as X,i as le,j as K,k as ce,l as ue,Z as B,T as C,n as ee,p as k,q as M,t as H,F as A,v as x,x as de,y as F,r as q,w as y,b as f,z as pe,A as me,C as fe,D as te,E as ge,d as S,G as d,H as R,I as L,J as ve,K as be,L as ye,M as he,N as Ce,O as D}from"./index-Cal_NYT1.js";import we from"./LoadingPokebola-DdDRZsDA.js";const ne=re("pokeStore",()=>{const e=w([]),t=w(new Set),n=w(!0);return{pokemons:e,favorites:t,loading:n,fetchPokemons:async()=>{try{const p=Math.ceil(22.36),a=[],$=[];for(let b=0;b<p;b++)$.push(fetch(`https://pokeapi.co/api/v2/pokemon?limit=50&offset=${b*50}`).then(u=>u.json()).then(u=>a.push(...u.results)));await Promise.all($);const E=await Promise.all(a.map(async b=>{const u=await fetch(b.url).then(I=>I.json());return{...b,image:u.sprites.front_default,weight:u.weight,height:u.height,types:u.types.map(I=>I.type.name).join(", ")}}));e.value=E}catch(c){console.error("Error al obtener la lista de Pokémon:",c)}finally{n.value=!1}},toggleFavorite:c=>{t.value.has(c)?t.value.delete(c):t.value.add(c)},filteredPokemons:c=>c?e.value.filter(h=>t.value.has(h.name)):e.value}});var $e=({dt:e})=>`
.p-toast {
    width: ${e("toast.width")};
